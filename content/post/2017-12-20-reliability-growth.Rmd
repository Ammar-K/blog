...
<img src="~/R/blog/static/img/RGAheader.jpg">
...
### Weibull
Weibull analysis is a parametric analysis that give two parameters $\beta$ and $\eta$ so called shape (slope) and scale (characteristic life) respectively .
The weibull analysis is a powerful reliability distribution used to model degradation process (e.g. fatigue, corrosion, abrasion, etc). and since it's used to model failure mode this imply it's weakness of modelling mixed failure modes not to mention a whole equipment or a system.


## when to use it ?
This bring us to the usage of the RGA, which is used to assess a whole equipment or system.
RGA is originally used to test the product reliability under design, after testing the first prototype the tester will check if the reliability meeting the requirement (using destructive testing or simulation) if not a modification will be done to improve the reliability and the test will take place again and the RGA is used to measure the improvement over time and to interpolate the future improvement (when and how many iteration to meet the target) this usually done in Accelerated Life Testing Analysis (ALTA) following the "IEC 61014 programmes for reliability growth".
...
# RGA for a multiple systems

Performing RGA for all the RCM systems can be a logical step to evaluate how is our RCM effecting the failure rate and can tell in numbers if the RCM efforts sucessed or not.
...
So for a company with 150 RCM systems you need to perform 150 RGAs. which sound silly and hideous, but don't worry there are a workaround.

## The challanage of multible systems 

While analyzing the reliability of a single system sound to be a tough task (if we do it in the right way) analyzing more than 100 systems sound to be crazy. but in the age of data science and big data this seem to be a perfect opportunity for [automation of knoweldge work](https://www.mckinsey.com/business-functions/digital-mckinsey/our-insights/disruptive-technologies) and use of dormant data.
...
I'll assume you are using some kind of CMMS system (SAP in my case) where all of the failure records exists, now you might be lucky enough to have an SQL interface (Meridum in my case) or you are familiar with the system to be able to get a table looking like the one below.
...
If you are using meridum you use the code below.
...
* Check the activity: you may want to limit your data to (repair and replace) something like calibration or cleaning doesn't sound as failure of function.

* Adding PM01 notifications: Some of the PM01 are valid failure of function you might want to add these after the limiting the activity in the point before, this is depending on hoe your people treating their CMMS in some cases you might end up with 0 PM02(break down) and that not because you have reliable assets!

* Filter the repairable parts: When there is no parts listed in the parts field it can be wise to remove this entry, anyhow this again depend on how the people using the system, is it usual to have empty field ? or that mean to replacement done ?

* Text mining: based on the patterns you see in work orders you can set some filtration rules on the work order text(e.g. "paint", "painting","lighting","cleaning" when people used to raise notification on equipment instead of functional location). this is the biggest of field of improvement for the data quality.
**If you wait for the data to be clean by itself then be ready to take blind decisions**


### Step 3: Export the result
After you have established a query that clean your data now you can export your data to spread sheet. in future you should start from this point when you want to update your result unless you wanted to adjust your filtration.
### Step 4: Create your RGA code
In my case im using R code, the could is simple enogh for the begineer to understand.
...
Having your code now you can test result, i sugest to go the link in the fig 1.1 and check one of the example and see if your data matching the result. herewith a starter code for that.
...
the first function is "sapply" what want to do is to take list with multiple nodes each node represent a table and every table represent a single RCM system. 
...
While this sound a lot of headache at the first glance it still a very effective and highly customizable way to do your day to day activities. 
Probably you will feel demoralized when your code don't work -which will happen in the first time- but it's a skill that can be easily adapted and used on different cases. 
the code eventually will be less than 20 lines, an average coder can do it in one hour.
and if you have a doupt you can reach me for sure.
